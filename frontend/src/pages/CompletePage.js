...
export default function CompletePage() {
  ...
  const { deleteCart, addOrderHistory, getCart, cart, totalPrice } =
  ...
  useEffect(() => {
    const addHistoryAndDeleteCart = async () => {
      try {
        await addOrderHistory(cart, totalPrice);
        await deleteCart();
      } catch (error) {
        console.error(error);
      }
        ...
        {intentId && (
          ...
          >
            ...
            <svg
              ...
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              style={{ paddingLeft: "5px", marginRight: "20px" }}
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
